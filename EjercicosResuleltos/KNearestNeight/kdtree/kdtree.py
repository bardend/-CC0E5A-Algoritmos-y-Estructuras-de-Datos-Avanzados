...
def wiht_out_median(points):
    ...
    right_points = points[median_index + 1:]  # Excluye la mediana.
...
class _Node:
    ...
    """
    def __init__(self, points, dimensionality, depth=0, median_method = True):
        self._depth = depth
        ...
        if not points:  # Nodo vacío.
            ...
            if median_method :
                ...
                med, left_pts, right_pts = wiht_out_median(points)

            self._point = med
            self._left = _Node(left_pts, dimensionality, depth + 1, median_method= median_method)
            self._right = _Node(right_pts, dimensionality, depth + 1, median_method = median_method)
    ...
    def kNearestNeighbours(self, target_point, k):
      ...
      self._kNearestNeighbours(target_point, k, heap)
      return heap

    def _kNearestNeighbours(self, target_point, k, heap):
      """Busca los k vecinos más cercanos recursivamente."""
      if self.isEmpty():
          return 
      
      current_dim_val = self.dim
      coord_to_target = target_point.coordinate(current_dim_val)
      coord_node = self._point.coordinate(current_dim_val)

      closest_branch = self._left if coord_to_target < coord_node else self._right
      further_branch = self._right if coord_to_target < coord_node else self._left

      if closest_branch : 
        closest_branch._kNearestNeighbours(target_point, k, heap)

      if heap.__sizeof__() < k :
        if further_branch : 
          further_branch._kNearestNeighbours(target_point, k, heap)
      else:
        dis_to_hiperplane = abs(coord_to_target-coord_node)
        if heap.peek_priority() > dis_to_hiperplane : # Puede existir algun punto
          heap.top()
          further_branch._kNearestNeighbours(target_point, k, heap)
      
      #Ahora reviso mi nodo actual y pusheo la informacion a mi padre

      if heap.__sizeof__() < k : 
        heap.insert(self._point, self._point.distanceTo(target_point))
      else:
        dis_to_me = self._point.distanceTo(target_point)
        if heap.peek_priority() > dis_to_me :
          heap.top() 
          heap.insert(self._point, dis_to_me)
...
class KdTree:
    ...
    """
    def __init__(self, points=None, median_method = True):
        """
        ...
        if points:
            ...
            if points:
                ...
                self._root = _Node(points, self._K, depth=0, median_method=median_method)
            else:
