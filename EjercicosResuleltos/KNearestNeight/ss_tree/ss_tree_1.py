...
class SsTree:
    ...
    def k_nearest_neighbours(self, target: Point, k: int) -> List[Point]:
        if self.size == 0 or target.dimensionality != self._k:
            return None
        print("vamos por el root")
        return self._root.k_nearest_neighbour(target, k)
...
class _Node:
    ...
    def validate_candidate(self, candidate, distance, k, heap) :
        if heap.__len__() < k:
            heap.insert(candidate, distance)
        elif distance < heap.peek_priority():
            heap.pair_top()
            heap.insert(candidate, distance)

    def k_nearest_neighbour(self, target: Point, k: int) -> List[Point]:

        heap_walk = DWayHeap(branching_factor= 2)
        heap_target = DWayHeap(branching_factor= 2)
        inf = 100000000
        heap_walk.insert(self, inf)

        while not heap_walk.is_empty():
          dis, value = heap_walk.pair_top() #toma y elimina
          if isinstance(value, Point):
            self.validate_candidate(value, dis, k, heap_target)
            continue
          # Es nodo o es intern o leaf
          if value.is_leaf:
            for p in value.points:
              ds = target.distance_sq_to(p)
              self.validate_candidate(p, ds, k, heap_walk)
            ...
            for child in value.children:
              if child.centroid is None:
                continue
              lb_child = max(0.0, target.distance_sq_to(child.centroid) - child.radius**2)
              self.validate_candidate(child, lb_child, k, heap_walk)

        ret = []
        while(not heap_target.is_empty()):
            ret.append(heap_target.top())
        return ret[::-1]