...
import random
import argparse
from ss_tree_1 import SsTree, Point
...
def benchmark_construction(points: list[Point], max_points: int, use_variance:bool = True) -> float:
    """Mide el tiempo para construir un SsTree con los puntos dados."""
    start = time.perf_counter()
    _ = SsTree(points, max_children=max_points)
...
def benchmark_insertion(points: list[Point], max_points: int) -> float:
    ...
    tree = SsTree([], max_children=max_points)
    start = time.perf_counter()
    for p in points:
        tree.add(p)
    return time.perf_counter() - start
...
def benchmark_queries(tree: SsTree, queries) -> float:
...
def main():
    ...
    parser.add_argument("-n", type=int, default=40000, help="Número de puntos a generar")
    parser.add_argument("-d", type=int, default=2, help="Dimensionalidad de los puntos")
    #parser.add_argument("-q", type=int, default=1000, help="Número de consultas NN")
    parser.add_argument("-q", type=int, default=100, help="Número de consultas NN")
    parser.add_argument("-m", type=int, default=10, help="max_points por nodo")
    parser.add_argument("-vs", type=bool, default=False, help="Use variance splitting")
...
'''
    tree = SsTree(points, max_children=args.m)
